...
from cut_resize_tools import slide, remove_nan, parallel_processing, process_data_segment, normalization, select_conv, proccess_npyAppend
...
def process_fits_data(fits_path, cut_size_list, sch_ii, ech_ii, vsmooth, thresh, sigma, sch_rms, ech_rms, integrate_layer_num, output_dir, obj_size, obj_sig):
    ...
    for pix in cut_size_list:
        ...
        print("Starting convolution and resize")
        conv_resize_list = []
        for _data in tqdm(processed_list):
            ...
            conv_resize_list.append(_conv_resize_data)

        if 
        processed_list = normalization(conv_resize_list)
        cut_resize_all_list += processed_list
        print(f"Processed data shape for {pix} pixels: {processed_list[-1].shape}\n")
    
    output_file = f"{output_dir}CygnusX_cut_resize_to_{obj_size}x{obj_size}"
