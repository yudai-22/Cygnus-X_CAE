...
from cut_resize_tools import slide, remove_nan, parallel_processing, process_data_segment, normalization, select_conv, proccess_npyAppend, maximum_value_determination
...
OUTPUT_DIR = "/home/filament/fujimoto/Cygnus-X_CAE/data/zroing_resize_data/resize_data/"
...
def process_fits_data(fits_path, cut_size_list, sch_ii, ech_ii, vsmooth, thresh, sigma, sch_rms, ech_rms, integrate_layer_num, percentile, output_dir, obj_size, obj_sig):
    ...
    max_thresh = maximum_value_determination(raw_data, sch_ii, vsmooth, ech_ii, sch_rms, ech_rms, sigma, thresh, integrate_layer_num, obj_size, obj_sig, percentile)    
    ...
    for pix in cut_size_list:
        ...
        for _data in tqdm(processed_list):
            ...
            _conv_resize_data = resize(_conv_data, (obj_size, obj_size))
            _conv_resize_MaxThresh = np.clip(_conv_resize_data, a_min=None, a_max=max_thresh)
            conv_resize_list.append(_conv_resize_MaxThresh)
        
        processed_list = normalization(conv_resize_list, max_thresh)
        cut_resize_all_list += processed_list
        print(f"Processed data shape for {pix} pixels: {processed_list[-1].shape}\n")
    
    output_file = f"{output_dir}CygnusX_cut__change_max_resize_to_{obj_size}x{obj_size}"
    proccess_npyAppend(output_file, cut_resize_all_list)
    # np.save(output_file, np.array(cut_resize_all_list))
